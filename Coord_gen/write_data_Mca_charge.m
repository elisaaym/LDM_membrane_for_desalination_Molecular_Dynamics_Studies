...
size_H2O_y =40*a; %Must be a multiple of a2 to ensure good lattice with periodic BC. Even multiple so we can define z properly
size_H2O_x = 45*b*sin(gamma/180*pi); %Multiple of a1
size_H2O_z = 120;
n_y = ceil(size_H2O_y/a);
n_x = ceil(size_H2O_x/(b*sin(gamma/180*pi)));
...
molar_mass_F = 18.998403;

%carbon carbon interactions measured by tersoff potential, not through
%bonds
[ zm, ym, xm, atomtype_M, charge_M, n_ym, n_xm,n_M ] = construct_MXeneTi3C2O2ca( size_H2O_y,size_H2O_x , 0, 0, 0);
...
mass_array = [molar_mass_O, molar_mass_H, molar_mass_NA, molar_mass_CL, molar_mass_C, molar_mass_Ti, molar_mass_C, molar_mass_F];
...
for i=1:n_M
    ...
    fprintf(fid, formatspec, atom_ID, mol_ID, atomtype, charge(i), xm(i), ym(i), zm(i));
    atom_ID = atom_ID + 1;
    mol_ID = mol_ID + 1; 
end
