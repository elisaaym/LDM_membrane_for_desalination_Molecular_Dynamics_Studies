...
size_H2O_y = 37.8; %Must be a multiple of 3*1.42 to ensure good graphene lattice with periodic BC. Even multiple so we can define z properly
size_H2O_z = 51.9;
size_H2O_x = 56.715*2;
dia_x =6.9818;
...
num_mol_1D_H2O_z = ceil(num_mol_1D_H2O_y*const_a)+1;
...
num_C=num_C*2;
slitsize =10.3-0.5; %due to crystal struture, need to start at y=0.5.


[ xm, ym, zm, atomtype_M, charge_M, n_ym, n_zm,n_M ] = construct_MXeneTi3C2O2( size_H2O_y,size_H2O_z , 3+size_H2O_x+3, 0.5, 0, slitsize );


molar_mass_C = 12.0107;
molar_mass_Ti = 47.867;
...
num_atoms = num_atoms+num_mol_H2O_p*3+num_NACL*2 + num_C+n_M;
...
fprintf(fid, formatspec, -20,3+size_H2O_x+3+dia_x+3+space_H2O_x*layers+3+120);
...
mass_array = [molar_mass_O, molar_mass_H, molar_mass_NA, molar_mass_CL, molar_mass_C, molar_mass_Ti, molar_mass_C, molar_mass_O];
...
for i=1:n_M
    ...
    fprintf(fid, formatspec, atom_ID, mol_ID, atomtype, charge_M(i), xm(i), ym(i), zm(i));
    atom_ID = atom_ID + 1;
    mol_ID = mol_ID + 1; 
 end
